...
const audioPrefix = Object.freeze(new Uint8Array(ArrayBuffer.fromString('{"realtimeInput":{"audio":{"mimeType":"audio/pcm;rate=16000","data":"')), true);
...
class GoogleGeminiLiveTranscription extends GoogleGeminiLiveModel {
    ...
    'serverContent'(data) {
        ...
        if (inputTranscription?.text) {
            trace(`[inputTranscription](${data.inputTranscription.languageCode}) ${data.inputTranscription.text}\n`)
        ...
        if (outputTranscription?.text) { 
            trace(`[outputTranscription](${data.outputTranscription.languageCode}) ${data.outputTranscription.text}\n`)
